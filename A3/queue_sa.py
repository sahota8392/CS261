...
# Due Date: 11/06/23
# Description: Implementation of Queue ADT utilizing Static Array
...
class Queue:
    ...
    def enqueue(self, value: object) -> None:
        """
        Adds new value to end of queue (append) - O(1) amortized
        """
        # if the current size equals max size of self._sa.length() call double_queue to double the queue size
        if self._current_size == self._sa.length():
            self._double_queue()

        insertion = (self._back + 1) % self._sa.length()  # when tail wraps near end, circular buffer occurs to wrap
        self._sa.set(insertion, value)  # call set method to insert value at insertion index
        self._back = insertion  # update self._back
        self._current_size += 1  # update current size to check if we reach max size
    ...
    def dequeue(self) -> object:
        ...
        """
        # empty queue will raise exception since dequeue isn't possible
        if self._current_size == 0:
            raise QueueException

        deletion = (self._front + 1) % self._sa.length()        # retrieve first element in queue
        val = self._sa.get(deletion - 1)
        self._front = deletion                                  # update self._front to increment
        self._current_size -= 1                                 # current_size will decrement
        return val

    def front(self) -> object:
        """
        Return value of the front element of queue without removing.
        If empty queue, raise exception - O(1)
        """
        if self._current_size == 0:
            raise QueueException

        # returns the top element without any deletion
        top_element = (self._front + 1) % self._sa.length()
        value = self._sa.get(top_element - 1)
        return value
    ...
    def _double_queue(self) -> None:
        """
        Double the queue size if the queue is full
        """
        resize = 2 * self._sa.length()          # resize by doubling the initial size of StaticArray
        new_list = StaticArray(resize)          # new list is created with the StaticArray but doubled

        # iterate over elements in queue copying over to the new list by index and using circular buffer behavior
        for i in range(self._current_size):
            new_list.set(i, self._sa[(self._front + i) % self._sa.length()])

        # original static array is updated to the new list that was doubled
        self._sa = new_list
        self._front = 0
        self._back = self._current_size - 1     # back index updated to point to last element in new queue
